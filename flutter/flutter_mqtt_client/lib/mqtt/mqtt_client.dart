...
///测试服
String _serverTest = 'mqtt://thermal.luci.ai';

///线上
String _serverOnline = 'test.mosquitto.org';
...
MqttServerClient _client = MqttServerClient.withPort(_server, clientID, _port);
...
class MqttUtils {
  ...
  Future<MqttServerClient> testConnect() async {
    _client.port = port; //對應 port
    _client.setProtocolV311(); //設置 mqtt 的版本
    ...
    if (_client.connectionStatus.state == MqttConnectionState.connected) {
      ...
      print(
          'ERROR client connection failed - disconnecting, state is ${_client.connectionStatus.state}');
      _client.disconnect();
    }

    //以下是訂閱主題並監聽的該主題的訊息
    _client.subscribe("Test", MqttQos.atLeastOnce); //這裡訂閱了 Test 主題。
  ...
  void test() async {
    int res = await connect(username, password);
    if (res != 0) {
    ...
    String jsonString =
        "{\"device_id\":\"${MQTTTopic.device_id}\",\"name\":\"test_name\"}";
  ...
  Future<int> connect(String IMEI, String deviceUid) async {
    ...
    if (_client.connectionStatus.state == MqttConnectionState.connected) {
      ...
      print(
          'MqttUtils::ERROR $_server client connection failed - disconnecting, status is ${_client.connectionStatus}');
      _client.disconnect();
      return -1;
    }
    ...
    _client.updates.listen((List<MqttReceivedMessage<MqttMessage>> c) {
      ...
      print(
          'MqttUtils::Change notification:: topic is <$topic>, payload is <-- $dataJson -->');
  ...
  bool publishMessage(String topic, String jsonString,
    ...
    print(
        'MqttUtils::publishMessage topic = $topic, MqttQos = $qualityOfService, '
        'jsonString = $jsonString');

    if (!isConnected()) {
      print(
          'MqttUtils::publishMessage topic = $topic---- client is not conntected');
      return false;
    }
  ...
  bool subscribe(String topic,
    ...
    if (!isConnected()) {
      print(
          'MqttUtils::subscribe  topic = $topic ---- client is not conntected');
      return false;
    }
    ...
    if (callBack != null) {
      ...
      print(
          "_topicCallBackMap subscribe put  key = $topic, callBack = ${callBack == null ? null : callBack.toString()}");
    }
  ...
  bool unsubscribe(String topic) {
    ...
    if (!isConnected()) {
      print(
          'MqttUtils::Unsubscribing  topic = $topic---- client is not conntected');
      return false;
    }
